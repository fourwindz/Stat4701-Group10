...
By plotting the data we have, we see a clear general trend that flooding is getting more severe year after year - the duration and severity level are both increasing. The months with the most severe floods are December-January, April, and August. We also notice that there are at least one extremely severe flood that killed hundreds people in year 1991, 2004, and 2008. Further, there are most people displaced in 1999 and 2000 (while the number of people killed in those years are low), which imply that displacing residents in the flooded area in time can effectively reduce the level of fatality of floods. So we can save people by predicting floods and displacing people. Now let's take a look at each graph individually:
...
It is interesting to see that the average number of people killed by flood is the highest in December. We suspect that one of the most severe flood that had enormous amount of people skilled happened in December, which then pull up the average in December. The month with the second largest amount of people killed by flood is April, which makes sense because April is also one of the month we saw above that has the most sever floods in terms of duration.
...
From the plot, we see that there are most people displaced in average in year 1995, 1999, 2000. These years are not the years with most people skilled by floods, which may imply that displacing residents of the flooded area on time can significantly reduce the number of people skilled by flood. 
